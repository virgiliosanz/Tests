...
// functions (i.e. navigation device management) or to be executed inside a
// menu.
...
#define TSFORM(b, s, l, c)                                                     \
  memset(b, 32, l);                                                            \
  strncpy_P(b, (const char PROGMEM *)s,                                        \
            min(col, strlen_P((char PROGMEM *)s)));                            \
  b[strlen(b)] = ' ';                                                          \
...
#define FSFORM(b, s, l)                                                        \
  memset(b, 32, l);                                                            \
  memcpy_P(b, (const char PROGMEM *)s,                                         \
           min(l, strlen_P((const char PROGMEM *)s)));                         \
  buf[l] = NULL;                                                               \
  lcd->print(b);

#define ERROR(a) MW_error = a

#define BLANKLINE(b, r, c)                                                     \
  memset(b, 32, c);                                                            \
  b[c] = NULL;                                                                 \
  lcd->setCursor(0, r);                                                        \
  lcd->print(b)
...
const uint8_t c0[8] = {B00000, B00000, B00001, B00010,
                       B10100, B01000, B00000, B00000};
const uint8_t c1[8] = {B00000, B11100, B00100, B00100,
                       B10101, B01110, B00100, B00000};
const uint8_t c2[8] = {B00000, B01010, B11111, B01110,
                       B11111, B01010, B00000, B00000};
// const uint8_t c3[8]={B00100, B01110, B11111, B00000, B00000, B00000, B11111,
// B00000};
...
void _menu::addVar(MW_TYPE t, int *v, int low, int up, int incr) {
  ...
  if (t != MW_AUTO_INT)
    ...
    ((_var *)var)->val = v;
    ((_var *)var)->lower = malloc(sizeof(int));
    if (((_var *)var)->lower != NULL)
      VINT(((_var *)var)->lower) = low;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->upper = malloc(sizeof(int));
    if (((_var *)var)->upper != NULL)
      VINT(((_var *)var)->upper) = up;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->incr = malloc(sizeof(int));
    if (((_var *)var)->incr != NULL)
      VINT(((_var *)var)->incr) = incr;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->old = malloc(sizeof(int));
    if (((_var *)var)->old != NULL)
      VINT(((_var *)var)->old) = VINT(((_var *)var)->val);
    else {
      ERROR(900);
      return;
    }
...
void _menu::addVar(MW_TYPE t, float *v, float low, float up, float incr) {
  ...
  if (t != MW_AUTO_FLOAT)
    ...
    ((_var *)var)->val = v;
    ((_var *)var)->lower = malloc(sizeof(float));
    if (((_var *)var)->lower != NULL)
      VFLOAT(((_var *)var)->lower) = low;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->upper = malloc(sizeof(float));
    if (((_var *)var)->upper != NULL)
      VFLOAT(((_var *)var)->upper) = up;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->incr = malloc(sizeof(float));
    if (((_var *)var)->incr != NULL)
      VFLOAT(((_var *)var)->incr) = incr;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->old = malloc(sizeof(float));
    if (((_var *)var)->old != NULL)
      VFLOAT(((_var *)var)->old) = VFLOAT(((_var *)var)->val);
    else {
      ERROR(900);
      return;
    }
...
void _menu::addVar(MW_TYPE t, byte *v, byte low, byte up, byte incr) {
  ...
  if (t != MW_AUTO_BYTE)
    ...
    ((_var *)var)->val = v;
    ((_var *)var)->lower = malloc(sizeof(byte));
    if (((_var *)var)->lower != NULL)
      VBYTE(((_var *)var)->lower) = low;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->upper = malloc(sizeof(byte));
    if (((_var *)var)->upper != NULL)
      VBYTE(((_var *)var)->upper) = up;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->incr = malloc(sizeof(byte));
    if (((_var *)var)->incr != NULL)
      VBYTE(((_var *)var)->incr) = incr;
    else {
      ERROR(900);
      return;
    }
    ((_var *)var)->old = malloc(sizeof(byte));
    if (((_var *)var)->old != NULL)
      VBYTE(((_var *)var)->old) = VBYTE(((_var *)var)->val);
    else {
      ERROR(900);
      return;
    }
...
void _menu::addVar(MW_TYPE t, boolean *v) {
  ...
  if (t != MW_BOOLEAN)
    ...
    if (((_var *)var)->old != NULL)
      VBOOL(((_var *)var)->old) = VBOOL(((_var *)var)->val);
    else {
      ERROR(900);
      return;
    }
...
// get nav choice
  ...
  ret = usrNav.fl
            ? usrNav.fi()
            : scanNavButtons(); // internal method or user defined callback?
#else
  ret = usrNav.fi(); // user defined callback
#endif

  // ENTER MENU SCREEN IF ANY BUTTON IS PUSHED
  // if a button was pushed while running usrscreen or splash screen , skip last
  // button, switch to MENU mode, draw menu and exit
  if (((cur_mode == MW_MODE_USRSCREEN) || (cur_mode == MW_MODE_SPLASH)) &&
      (ret != MW_BTNULL)) {
  ...
  // else run the action associated to selected button
    ...
    if (bitRead(flags, FL_SPLASH) && (!bitRead(flags, FL_SPLASH_DRAW)) &&
        (lap1 < tm_splash)) {
    ...
    // USERSCREEN
    // if defined usrscreen & time since last button push > user defined time,
    // draw it
...
void menwiz::drawMenu(_menu *mc) {
  ...
  if ((mc->type == MW_ROOT) || (mc->type == MW_SUBMENU) ||
      (((_act *)mc->var)->type == MW_LIST)) {
    if (bitRead(flags, MW_MENU_INDEX)) {
      if (((_act *)mc->var)->type == MW_LIST) {
        top = 126;
      } else if (bitRead(m[((_option *)mc->o[mc->cur_item])->sbm].flags,
                         cur_user) == false) {
        top = 2;
      } else
        top = 126;
    ...
    for (i = 1, j = rstart; i < row; i++, j++) {
      if (j < rstop) {
        ...
        if (bitRead(mc->flags, MW_MENU_COLLAPSED)) {
          ...
          mn = &m[op->sbm];
          if (j == mc->cur_item) {
            if (bitRead(m[((_option *)mc->o[mc->cur_item])->sbm].flags,
                        cur_user) == false)
              buf[0] = 2;
            else
              buf[0] = 126;
          ...
          strncpy_P(&buf[1], (const char PROGMEM *)mn->label,
                    min(col, strlen_P((const char PROGMEM *)mn->label)));
          strcpy(sbuf, buf);
          if (mn->type == MW_VAR) {
            ...
            case MW_AUTO_INT:
              if (fl) {
                strcat(sbuf, ":[");
                strcat(sbuf, itoa(VINT(((_var *)mn->var)->val), buf, 10));
                strcat(sbuf, "]");
              } else {
                strcat(sbuf, ":");
                strcat(sbuf, itoa(VINT(((_var *)mn->var)->val), buf, 10));
            ...
            case MW_BOOLEAN:
              if (fl) {
                strcat(sbuf, ":[");
                strcat(sbuf, itoa(VBOOL(((_var *)mn->var)->val), buf, 10));
                strcat(sbuf, "]");
              } else {
                strcat(sbuf, ":");
                strcat(sbuf, itoa(VBOOL(((_var *)mn->var)->val), buf, 10));
            ...
            case MW_AUTO_FLOAT:
              if (fl) {
                strcat(sbuf, ":[");
                dtostrf(VFLOAT(((_var *)mn->var)->val), 0, MW_FLOAT_DEC, buf);
                strcat(sbuf, buf);
                strcat(sbuf, "]");
              } else {
                strcat(sbuf, ":");
            ...
            case MW_AUTO_BYTE:
              if (fl) {
                strcat(sbuf, ":[");
                strcat(sbuf, itoa(VBYTE(((_var *)mn->var)->val), buf, 10));
                strcat(sbuf, "]");
              } else {
                strcat(sbuf, ":");
                strcat(sbuf, itoa(VBYTE(((_var *)mn->var)->val), buf, 10));
            ...
            lcd->write((j == mc->cur_item)
                           ? 126
                           : (bitRead(m[op->sbm].flags, cur_user) ? 165 : 2));
...
void menwiz::drawVar(_menu *mc) {
  ...
  case MW_LIST:
    if (bitRead(mc->flags, MW_SCROLL_HORIZONTAL)) {
      ...
      strncpy_P(buf, (const char PROGMEM *)pgm_read_word(op->label),
                min(col, strlen_P((const char PROGMEM *)op->label)));
      strcpy(sbuf, "[");
      strcat(sbuf, buf);
      strcat(sbuf, "]"); //@
      SFORM(buf, sbuf, col);
...
void menwiz::navButtons(int btu, int btd, int bte, int btc) {
  ...
  if (btu != 0) {
    btx->BTU.assign(btu);
    btx->BTU.setMode(OneShot);
    btx->BTU.turnOnPullUp();
  }
  if (btd != 0) {
    btx->BTD.assign(btd);
    btx->BTD.setMode(OneShot);
    btx->BTD.turnOnPullUp();
  }
  if (bte != 0) {
    btx->BTE.assign(bte);
    btx->BTE.setMode(OneShot);
    btx->BTE.turnOnPullUp();
  }
  if (btc != 0) {
    btx->BTC.assign(btc);
    btx->BTC.setMode(OneShot);
    btx->BTC.turnOnPullUp();
  }
...
void menwiz::navButtons(int btu, int btd, int btl, int btr, int bte, int btc) {
  ...
  if (btl != 0) {
    btx->BTL.assign(btl);
    btx->BTL.setMode(OneShot);
    btx->BTL.turnOnPullUp();
  }
  if (btr != 0) {
    btx->BTR.assign(btr);
    btx->BTR.setMode(OneShot);
    btx->BTR.turnOnPullUp();
  }
  if (bte != 0) {
    btx->BTE.assign(bte);
    btx->BTE.setMode(OneShot);
...
void menwiz::actBTU() {
  ...
  if ((cur_menu->type != MW_VAR) ||
      (((_var *)cur_menu->var)->type == MW_LIST)) {
    cur_menu->cur_item = (cur_menu->cur_item - 1) < 0 ? (cur_menu->idx_o - 1)
                                                      : cur_menu->cur_item - 1;
...
void menwiz::actBTL() {
  ...
  if (((_var *)cm->var)->type == MW_AUTO_BYTE) {
    VBYTE(((_var *)cm->var)->val) =
        max((VBYTE(((_var *)cm->var)->val) - VBYTE(((_var *)cm->var)->incr)),
            VBYTE(((_var *)cm->var)->lower));
  } else if (((_var *)cm->var)->type == MW_AUTO_INT) {
    VINT(((_var *)cm->var)->val) =
        max((VINT(((_var *)cm->var)->val) - VINT(((_var *)cm->var)->incr)),
            VINT(((_var *)cm->var)->lower));
  } else if (((_var *)cm->var)->type == MW_AUTO_FLOAT) {
    VFLOAT(((_var *)cm->var)->val) =
        max((VFLOAT(((_var *)cm->var)->val) - VFLOAT(((_var *)cm->var)->incr)),
            VFLOAT(((_var *)cm->var)->lower));
    ...
    cm->cur_item = (cm->cur_item + 1) % (cm->idx_o);
           ...
           (MW_navbtn == 6)) {
    cur_menu->cur_item =
        (cur_menu->cur_item - 1 < 0)
            ? (strlen((char *)((_var *)cur_menu->var)->val) - 1)
            : (cur_menu->cur_item - 1);
    //@
  }
}
...
void menwiz::actBTR() {
  ...
  if (((_var *)cm->var)->type == MW_AUTO_INT) {
    VINT(((_var *)cm->var)->val) =
        min((VINT(((_var *)cm->var)->val) + VINT(((_var *)cm->var)->incr)),
            VINT(((_var *)cm->var)->upper));
  } else if (((_var *)cm->var)->type == MW_AUTO_BYTE) {
    VBYTE(((_var *)cm->var)->val) =
        min((VBYTE(((_var *)cm->var)->val) + VBYTE(((_var *)cm->var)->incr)),
            VBYTE(((_var *)cm->var)->upper));
  } else if (((_var *)cm->var)->type == MW_AUTO_FLOAT) {
    VFLOAT(((_var *)cm->var)->val) =
        min((VFLOAT(((_var *)cm->var)->val) + VFLOAT(((_var *)cm->var)->incr)),
            VFLOAT(((_var *)cm->var)->upper));
  } else if (((_var *)cm->var)->type == MW_BOOLEAN) {
    VBOOL(((_var *)cm->var)->val) = !VBOOL(((_var *)cm->var)->val);
  } else if ((((_var *)cm->var)->type == MW_LIST) && MW_invar)
    cm->cur_item = (cm->cur_item - 1) < 0 ? (cm->idx_o - 1) : cm->cur_item - 1;
           ...
           (MW_navbtn == 6)) {
    cur_menu->cur_item = (cur_menu->cur_item + 1) %
                         (strlen((char *)((_var *)cur_menu->var)->val));
    //@
  }
}
...
void menwiz::actBTC() {
  ...
  if ((cur_menu->type == MW_SUBMENU) || (cur_menu->type == MW_ROOT)) {
    ...
    if (cur_menu->type == MW_VAR) {
      if (bitRead(m[cur_menu->parent].flags, MW_MENU_COLLAPSED)) {
        if ((((_var *)cur_menu->var)->type != MW_LIST) &&
            ((((_var *)cur_menu->var)->type != MW_ACTION))) {
          cur_menu = &m[cur_menu->parent];
          MW_invar = false;
        } else {
          MW_invar = true;
        ...
        return;
      } else if ((((_var *)cur_menu->var)->type == MW_ACTION) &&
                 (!bitRead(cur_menu->flags, MW_ACTION_CONFIRM))) {
    ...
    if (((_var *)cur_menu->var)->type == MW_LIST) {
      ...
      VFLOAT(((_var *)cur_menu->var)->old) =
          VFLOAT(((_var *)cur_menu->var)->val);
...
int menwiz::getErrorMessage(boolean fl) {
  /*
    if (fl){
      switch(MW_error){
        case 0:   break;
        case 100: Serial.println(F("E100-Too many items. Increment
    MAX_MENU"));break;
        case 130: Serial.println(F("E130-Invalid buttons number: allowed 4 or
    6"));break;
        case 200: Serial.println(F("E200-Root undefined"));break;
        case 105: Serial.println(F("E105-Too many items. Increment
    MAX_OPTXMENU"));break;
        ...
        case 410: Serial.println(F("E410-Behavihour available only with 6
    buttons. ignored"));break;
