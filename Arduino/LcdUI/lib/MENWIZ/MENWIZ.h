...
// comment if you do'nt want to use the readEeprom and writeEeprom methods!
#define EEPROM_SUPPORT

 // comment if you do'nt want to use the Button methods!
...
#define MENWIZ_VER "1.3.2 beta"

// SIZES (DIMENSIONAL LIMITS OF STATICALLY ALLOCATED STRUCTURES)
// ---------------------------------------------------------------------------
// maximum number of nodes (absolute supported max number of addMenu calls)
#define MAX_MENU 15
// maximum number of options/submenus for each node (max number of addItem call
// for each menu item)
...
public:
  ...
  // if arg=true, err message is printed to the default Serial terminal,
  // otherwise the function returns error code only
  int getErrorMessage(boolean);
